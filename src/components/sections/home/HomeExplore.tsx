...
export default function HomeExplore({ inMovie }: { inMovie?: boolean }) {
  ...
  const results = useQueries({
    queries: MOVIE_GENRES.map((genre) => ({
      ...
      queryFn: async () => {
        const res = await API.get(`/discover/movie?with_genres=${genre.id}`);
        return res.data;
      },
    })),
  });
  ...
  return (
    <section className={`${inMovie ? "" : "mb-37.5"} `}>
      <div className="container">
        <div className="mb-20 flex items-end justify-between">
          {inMovie ? (
            <Subtitle text="Our Genres" />
          ) : (
            <div className="">
